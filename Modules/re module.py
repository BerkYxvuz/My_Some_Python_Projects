...
# result = re.findall("Python",str) #belirtilen stringi arar
# result = len(result) belirtilmiş stringin kaç tane olduğunu yazar

# re.split()

# result = re.split(" ", str) # belirtilen stringe göre böler
# result = re.split("R", str)

# re.sub()

# result = re.sub(" ","-",str) belirtilen stringin değişmesini sağlar
# result = re.sub("\s","-",str)

# re.search()

# result = re.search("Python",str) bu kod bulunup bulunmadığına bakar ve cümledeki yerinide belirtir

# result = result.span() toplamda ne kadar yer kapladığını yazar
# result = result.start() cümlede nerde başladığını söyler
# result = result.end() cümlede nerde bittiğini söyler
# result = result.group() başlangıcını ve bitişini döndürür
